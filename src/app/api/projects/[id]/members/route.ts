...
export async function POST(
  ...
  try {
    ...
    // Check current user access — same match rule as getProjectRoleForApi:
    // own clerk_id always counts; email only counts for an admin, or for an
    // unclaimed row created by the new-domain approval flow.
    ...
    const userMapping = allMappings?.find((m: any) => {
      ...
      return admin || (!m.clerk_id && m.granted_via === 'new_domain')
    })
    ...
    // Check if user already exists in users table. Scoped to approval_status
    // IS NOT NULL — that column is only ever set by the new-domain webhook,
    // so this only matches a genuine new-domain account. Without this:
    // (a) an old-domain-only row would match too, wiring this invite to a
    //     clerk_id that can never be claimed by their actual new-domain
