            ...
            <Tooltip>
              ...
              <TooltipContent side="right" className="max-w-xs">
                <p className="text-xs">
                  ...
                  <strong>Fixed</strong>: always wait for Min Delay silence.<br />
                  <strong>Dynamic</strong>: adjust delay based on context (requires turn detection model).
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
          <Select
            value={endpointing_mode ?? 'auto'}
            onValueChange={(value) =>
              onFieldChange('advancedSettings.session.endpointing_mode', value === 'auto' ? null : value)
            }
          >
            <SelectTrigger className="h-8 text-xs">
              <SelectValue />
