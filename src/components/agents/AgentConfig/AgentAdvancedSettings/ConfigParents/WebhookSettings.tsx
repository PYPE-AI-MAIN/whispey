...
import { WebhookConfig } from '@/lib/supplementalSettings'
...
interface WebhookSettingsProps {
  ...
  onDataLoaded: (data: WebhookConfig) => void
  agentId?: string
  projectId?: string
}
...
function WebhookSettings({
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data && data.data.length > 0) {
          ...
          const loadedState: WebhookConfig = {
  ...
  const updateHeaders = (newEntries: Array<{ key: string; value: string }>) => {
    ...
    newEntries.forEach(entry => {
      if (entry.key.trim()) headersObj[entry.key.trim()] = entry.value.trim()
    })
    applyChange({ headers: headersObj })
  }

  const addHeaderEntry = () => setHeaderEntries([...headerEntries, { key: '', value: '' }])
        ...
        <>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => {
                if (webhookUrlError) setWebhookUrlError('')
                applyChange({ webhookUrl: e.target.value })
              }}
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(value) => applyChange({ httpMethod: value })}
            >
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) => applyChange({ isActive: checked })}
              className="scale-75"
            />
          </div>
        </>
