...
/** The "why do you want access" form shown when a user without create-permission asks for it. */
function RequestAccessForm({ isMobile, reason, setReason, loading, success, error, onBack, onSubmit }: Readonly<RequestAccessFormProps>) {
  return (
    <>
...
/** Header shown above the two choice cards. */
function ChoiceScreenHeader({ isMobile }: Readonly<{ isMobile: boolean }>) {
  return (
    <DialogHeader className={`${isMobile ? 'px-4 pt-4 pb-3' : 'px-6 pt-6 pb-4'} flex-shrink-0`}>
...
/** Two skeleton cards shown while permissions are loading. */
function ChoiceCardsSkeleton({ isMobile }: Readonly<{ isMobile: boolean }>) {
  return (
    <>
      {[1, 2].map((i) => (
...
interface CreateAgentOptionProps {
  isMobile: boolean
  canCreatePypeAgent: boolean | undefined
  hasReachedLimit: boolean | undefined
  onCreateAgent: () => void
  onRequestAccess: () => void
}

/** The body copy for the "Create New Agent" card — one of three mutually exclusive states. */
function CreateAgentOptionCopy({ isMobile, canCreatePypeAgent, hasReachedLimit }: Readonly<Omit<CreateAgentOptionProps, 'onCreateAgent' | 'onRequestAccess'>>) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`${isMobile ? 'text-xs' : 'text-sm'} ${
            ...
            <CreateAgentOptionCopy isMobile={isMobile} canCreatePypeAgent={canCreatePypeAgent} hasReachedLimit={hasReachedLimit} />
          </p>
        </div>
      </div>
    </div>
...
/** Cancel-only footer for the main choice screen. */
function ChoiceScreenFooter({ isMobile, onClose }: Readonly<{ isMobile: boolean; onClose: () => void }>) {
  return (
...
const AgentChoiceScreen: React.FC<AgentChoiceScreenProps> = ({
  ...
  return (
    <>
      <ChoiceScreenHeader isMobile={isMobile} />

      {/* Content */}
      <div className={`flex-1 ${isMobile ? 'px-4 py-3' : 'px-6 py-4'}`}>
        <div className="space-y-3">
          {permissionsLoading ? (
            <ChoiceCardsSkeleton isMobile={isMobile} />
          ) : (
            <>
              <CreateAgentOption
                isMobile={isMobile}
                canCreatePypeAgent={canCreatePypeAgent}
                hasReachedLimit={hasReachedLimit}
                onCreateAgent={onCreateAgent}
                onRequestAccess={() => setShowRequestForm(true)}
              />
              <ConnectAgentOption isMobile={isMobile} onConnectAgent={onConnectAgent} />
            </>
          )}
        </div>
      </div>

      <ChoiceScreenFooter isMobile={isMobile} onClose={onClose} />
    </>
  )
}
