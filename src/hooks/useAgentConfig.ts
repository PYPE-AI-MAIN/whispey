 ...
 *
 * update_status is served by the same server.py on both classic and docker
 * hosts (AgentUpdateManager tracks progress identically either way), so this
 * applies to both deploy targets — not docker-only.
 */
...
const saveAndDeployAgent = async (data: any) => {
  ...
  if (!response.ok) {
    ...
    // This lock is generic (AgentUpdateManager.is_update_in_progress has no
    // docker/classic distinction), so this applies to both deploy targets.
  ...
  // right away — for BOTH deploy targets, not just docker (update_agent_config
  // fires the same asyncio.create_task regardless of deployment_target) — so
  // poll until it actually finishes so callers (and the isPending-driven
  // loader) see the real completion, not just "request sent". Without this,
  // classic agents showed "done" the instant the request was accepted, while
  // the backend was still mid-way through stopping/restarting the subprocess.
