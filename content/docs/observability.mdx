...
whispey = LivekitObserve(
    ...
    observability_config={
        "capture_prompts": True,        # Store full conversation context
        "capture_configs": True,        # Store pipeline configuration
        "capture_tools": True,          # Track function executions
        "capture_costs": True,          # Calculate and track costs
        "capture_errors": True,         # Detailed error tracking
        "span_sampling": 1.0            # Sample rate for telemetry spans
    }
)
```
